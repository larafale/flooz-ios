...
//  Flooz
//
//  Created by olivier on 1/19/2014.
...
- (void)addFilter:(NSString *)image title:(NSString *)title target:(id)target action:(SEL)action colors:(NSArray *)colors {
...

	{
		...
		[button setImage:[UIImage imageNamed:[NSString stringWithFormat:@"%@-blue", image]] forState:UIControlStateSelected];
		[button setImage:[UIImage imageNamed:[NSString stringWithFormat:@"%@-blue", image]] forState:UIControlStateHighlighted];
...
- (void)didButtonTouch:(UIButton *)button {
	...
	for (UIView *filterView in filterViews) {
		...
		_button.titleLabel.font = [UIFont customContentLight:11];
	...
	[button setTitleColor:[UIColor customBlue] forState:UIControlStateNormal];
	button.titleLabel.font = [UIFont customContentRegular:12];
